...
return true;
    ...
    public boolean removeGreaterElement(int id, int user_id, int height) {
        try {
            var query = "select user_id from collection_elements where id="+id;
            var res = statement.executeQuery(query);
            if(res.next()){
                if (res.getInt("user_id") == user_id){
                    query = "delete from collection_elements where height>"+height;
                executeUpdate(query);
                return true;
            }}
        } catch (SQLException e) {
        ...
        public boolean updateElement(Person person, int user_id, int person_id) throws SQLException{
            try {
                var query = "select * from collection_elements where id="+user_id;
                var res = statement.executeQuery(query);
