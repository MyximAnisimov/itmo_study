...
public class Login extends AbstractCommand {
    ...
    public static UserIDNumber UID = new UserIDNumber(0);
    ...
    public boolean execute(String [] arguments){
        ...
        CustomConsole.printLn("Введите пароль пользователя: ");
            ...
            try {
                var response = (LoginCommandResponse) client.sendAndReceiveCommand(new LoginCommandRequest(user, password, 0, 0));
if(response.SuccLogin){
    UID.setGlobalUserId(response.user_id);
    UID.setCountable(response.count);
