...
public class CollectionManager {
    ...
    public List<Person> synchCollection;
...
load();
    ...
    public Person getById(int id){
        for (Person person: synchCollection) {
...
//}
    ...
    public int countElementsLessThanLocation(String location_name){
        int counter=0;
      for(int i=0;i<synchCollection.size();i++){
        }
          return counter;
    }
     ...
     */
    public Person first() {
        if (synchCollection.isEmpty()) return null;
        return sorted().get(0);
    }
    ...
    public int add(Person element) {
        var maxId = synchCollection.stream().filter(Objects::nonNull)
                .map(Person::getID)
                .mapToInt(Integer::intValue).max().orElse(0);
        var newId = maxId + 1;

        synchCollection.add(element.copy(newId));
        return newId;
    }
    ...
    public void remove(int id) {
        synchCollection.removeIf(person -> person.getID() == id);
    }
...
//    }
    ...
    private void load() {
        ...
        synchCollection =Collections.synchronizedList(new ArrayList<Person>());
        var dao111 = persistenceManager.loadLocation();
        var daos = persistenceManager.loadPerson();

        var products = daos.stream().map((dao) -> {
            Location location = null;
            if (dao.getLocation() != null) {
                location = new Location(
                        dao.getLocation().getX(),
                        dao.getLocation().getY(),
                        dao.getLocation().getZ(),
        ...
        synchCollection.addAll(products);
