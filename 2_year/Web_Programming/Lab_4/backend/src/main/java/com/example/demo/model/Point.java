...
public class Point implements Serializable{
    ...
    @Column @NotNull @Min(value = -5) @Max(value = 3) private double r;
    ...
    private boolean positiveRadius(double x, double y, double r){
        return ((  (x >= 0 && y >= 0 && x <= r && y <= r/2)// 1 Четверть
                || (x <= 0 && y >= 0 && x*x + y*y <= r*r)// 2 Четверть
                || (x >= 0 && y <= 0 &&  y >= x - r && y >= -r && x <= r))// 4 Четверть
              ) && (r >= 0 && r <= 3);
    }
    ...
    private boolean negativeRadius(double x, double y, double r){
        ...
        double x1 = -x;
        double y1 = -y;
        return ((  (x1 <= 0 && y1 >= 0 && x1*x1 + y1*y1 <= r1*r1)// 2 Четверть
                || (x1 >= 0 && y1 >= 0 && y1 <= r1/2 && x1<= r1)// 3 Четверть
                || (x1 >= 0 && y1 <= 0 && x1 <= r1 && y1 >= -r1 && y1>=x1 - r1))// 4 Четверть
              ) && (r < 0 && r >= -5);
    }
