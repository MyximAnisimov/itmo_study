...
function set_r_value(flag_draw){
    if(flag_draw == 1){
        document.getElementsByName('r').forEach(function(link) {
            link.addEventListener('click', function() {
                // Передача нажатого элемента в функцию getValueOfCommandLink
                var value = clickedElement.getAttribute('value');
                value_r = value;
            });
        });
